...
APP_PATH="$ROOT_DIR/dist/LocalStereoTranscriberDesktop.app"
...
"$PYTHON_BIN" -m PyInstaller \
  ...
  --icon "$ICON_PATH" \
  --osx-bundle-identifier "com.arnoudvanrooij.localstereotranscriber.desktop" \
  --name "LocalStereoTranscriberDesktop" \
  --add-data "$ROOT_DIR/assets:assets" \
  --add-data "$ROOT_DIR/streamlit_app.py:." \
  --add-data "$ROOT_DIR/transcribe_dual_channel_local.py:." \
  "$ROOT_DIR/packaging/desktop_runtime_wrapper.py"

set_plist_value "$PLIST_PATH" "CFBundleDisplayName" "Local Stereo Transcriber"
set_plist_value "$PLIST_PATH" "CFBundleShortVersionString" "1.0.0"
set_plist_value "$PLIST_PATH" "CFBundleVersion" "1.0.0"

echo "Built app: $ROOT_DIR/dist/LocalStereoTranscriberDesktop.app"
